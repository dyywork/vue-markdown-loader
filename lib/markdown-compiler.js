...
const getString = function (obj) {
  ...
  return str.replaceAll(/\"function|\'function/g, 'function').replaceAll(/\}\"|\}\'/g, '}').replaceAll(/return/g, 'return ')
}
 ...
 */
var renderVueTemplate = function(html, wrapper, opts) {
  ...
  if (opts.script && $('script').length > 0) {
    var scriptStr = methodStr = createdStr = beforeCreateStr = mountedStr= beforeMountStr =watchStr = computedStr = ''
    var dataObj = {}
    let watchObj = {}
    let computedObj = {}
    var methodObj = {}
    for (let i = 0; i < $('script').length; i++) {
      var fn = new Function("return " + $.html($('script')[i]).replace(/<script>/g, '').replace(/<\/script>/g, '').replaceAll(/\n/g, '').replace(/export default/, '').replaceAll(/ /g, ''))
      dataObj = Object.assign(dataObj, fn().data())
      watchObj = Object.assign(watchObj, fn().watch)
      computedObj = Object.assign(computedObj, fn().computed)
      methodObj = Object.assign(methodObj, fn().methods)
      if (fn().beforeCreate) {
        beforeCreateStr += fn()?.beforeCreate.toString().match(/beforeCreate\(\)\{(\S*)\}/)[1]
      }
      if (fn().created) {
        createdStr += fn()?.created.toString().match(/created\(\)\{(\S*)\}/)[1]
      }
      if (fn().beforeMount) {
        beforeMountStr += fn()?.beforeMount.toString().match(/beforeMount\(\)\{(\S*)\}/)[1]
      }
      if (fn().mounted) {
        mountedStr += fn()?.mounted.toString().match(/mounted\(\)\{(\S*)\}/)[1]
      }
    }
    watchStr = getString(watchObj)
    computedStr = getString(computedObj)
    for(let i in methodObj){
      methodStr += methodObj[i].toString()+ ','
    }
    scriptStr = 'export default {data() {return ' + JSON.stringify(dataObj) +'},watch:'+ watchStr +',computed:'+ computedStr +',methods:{'+ methodStr+ '},beforeCreate(){'+ beforeCreateStr +'},created(){'+ createdStr +'},beforeMount(){'+ beforeMountStr +'},mounted(){'+ mountedStr +'}}'
    $('script')[0].children[0].data = scriptStr
  }
