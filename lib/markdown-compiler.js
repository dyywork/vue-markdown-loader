...
const getString = function (obj) {
  ...
  return str.replaceAll(/\"function|\'function/g, 'function').replaceAll(/\}\"|\}\'/g, '}')
}
...
var renderVueTemplate = function(html, wrapper, opts) {
  ...
  if (opts.script && $('script').length > 0) {
    var scriptStr = methodStr = createdStr = beforeCreateStr = mountedStr= beforeMountStr =watchStr = ''
    var dataObj = {}
    let watchObj = {}
    var methodObj = {}
    ...
    scriptStr = 'export default {data() {return ' + JSON.stringify(dataObj) +'},watch:'+ watchStr +',methods:{'+ methodStr+ '},beforeCreate(){'+ beforeCreateStr +'},created(){'+ createdStr +'},beforeMount(){'+ beforeMountStr +'},mounted(){'+ mountedStr +'}}'
